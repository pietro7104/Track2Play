...
def CompileUser(user):
        ...
        for game in games:
            ...
            cursor.execute("SELECT genre_name FROM GameGenre WHERE (GameID=%s)",(str(game[0]),))
            genres = cursor.fetchall()
        
            for genre in genres:
                cursor.execute("SELECT * FROM UserGenre WHERE (UserID=%s AND genre_name=%s)",(str(user[0]), str(genre[0])))
                user_genre = cursor.fetchone()
                if (user_genre is None):
                        cursor.execute("INSERT INTO UserGenre VALUES (%s, %s, %s)",(str(user[0]), str(genre[0]), str(1)))
                        db.commit()
                else:
                    appearences = int(user_genre[2]) + 1
                
                    cursor.execute("UPDATE UserGenre SET appearences=%s WHERE (UserID=%s AND genre_name=%s)",(str(appearences), str(user[0]), str(genre[0])))
                    db.commit()
            
        
            cursor.execute("SELECT category_name FROM GameCategory WHERE (GameID=%s)",(str(game[0]),))
            categories = cursor.fetchall()
        
            for category in categories:
                cursor.execute("SELECT * FROM UserCategory WHERE (UserID=%s AND category_name=%s)",(str(user[0]), str(category[0])))
                user_category = cursor.fetchone()
                if (user_category is None):
                    cursor.execute("INSERT INTO UserCategory VALUES (%s, %s, %s)",(str(user[0]), str(category[0]), str(1)))
                    db.commit()
                else:
                    appearences = int(user_category[2]) + 1
    
                    cursor.execute("UPDATE UserCategory SET appearences=%s WHERE (UserID=%s AND category_name=%s)",(str(appearences), str(user[0]), str(category[0])))
                    db.commit()
        
        
            cursor.execute("SELECT tag_name, votes FROM GameTag WHERE (GameID=%s)",(str(game[0]),))
